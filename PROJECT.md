...
Given a ticker, produce a **probability distribution of intrinsic value** — a distribution, not a number — computed by a fully transparent, reproducible pipeline over company-reported financials, with fair value estimated per scenario and aggregated statistically.
...
| P3 | **The pipeline runs even when the LLM fails.** Deterministic fallback rules (keyword/pattern matching) guarantee schema coverage; the LLM adds coverage breadth and confidence, never correctness. |
...
- **D11. Deterministic fallback path is mandatory.** Without an LLM available, the pipeline yields lower-confidence but still valid output (keyword/pattern matching against canonical-field descriptors).
...
6. **Graceful LLM-abstention.** A fully functional degraded mode (P3, D11) — the pipeline's correctness is *independent* of LLM availability, which no LLM-analyst-style tool can claim.
...
- Auto-generation of financial statements from filings (out of scope: ingest assumes structured or semi-structured inputs).
...
|---|---|---|---|
| O1 | Historical backtest as the first validation step (score historical valuations vs. actual outcomes)? | Yes — identified in `PLAN.md` item 17; it is the difference between a validated tool and an engineering exercise. | v1 exit |
| O2 | Source of historical statements: vendor API (EDGAR/iXBRL) vs. file upload of PDFs/HTML? | File upload first (zero external dependency), vendor API as an extension. | Stage 1 build |
| O3 | Default industry-prior dataset (S&P GICS median financials): build or license? | Build a small curated set for the target universe; mark all priors `heuristic`. | Stage 2 build |
| O4 | Correlation estimation: historical covariance of assumptions vs. fixed heuristic matrix? | Hybrid: estimate where ≥ 5 years of co-movement data exists, otherwise heuristic matrix with source documented. | Stage 3 build |
| O5 | Minimum acceptable rejection rate before a scenario is flagged as mis-specified? | Propose hard threshold (e.g. 50%) + per-assumption warning (e.g. 20%). | Stage 3 build |
| O6 | Should scenario naming / assumption ranges be a JSON config per company, or always LLM-derived with deterministic defaults? | JSON override supported, LLM-derived as the interactive path, deterministic defaults as the offline path (mirrors D7/D11). | Stage 1–2 boundary |
...
1. The LLM reclassification stage produces a canonical schema with ≥ 90% field coverage and confidence scores per field, with deterministic fallback active in the absence of an LLM.
...
The **backtest (O1) is required before v1 is declared validated** — but v1 the *pipeline* can ship without it, clearly labeled as unvalidated in all output artifacts until O1 lands.
