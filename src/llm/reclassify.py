...
"""Reclassify one converted 10-K statement via Ollama.

Reads a record array from ``convert.py`` (``data/converted/``), sends it
with the perfected ``prompt.md`` system prompt to a local Ollama
...
The reclassified file is the tracked output of this pipeline stage;
...
INPUT_KEYS = ("id", "stmt", "order", "level", "label",
              "tag", "has_value")


def normalize_cik(cik: str) -> str:
    """Return ``cik`` zero-padded to 10 digits (EDGAR CIK form).
...
def load_records(path: Path) -> list[dict]:
    """Load and pre-flight-validate a converted record array.
    ...
    for i, rec in enumerate(raw):
        if not isinstance(rec, dict) or set(rec) != set(INPUT_KEYS):
            ...
            raise ValueError(f"{path}[{i}]: record must have exactly the "
...
_META_KEYS = {"raw_key", "value", "transform", "provenance",
              "derived_from", "rule_version", "notes", "label"}


def _leaf_paths(node, prefix: str, out: list[tuple[str, dict]]) -> None:
    """Recursively collect ``(dotted_path, metadata)`` for every leaf."""
...
def build_schema_block(schema: dict) -> str:
    """Render the canonical-schema dictionary for ``prompt.md`` injection.
    ...
    for name, spec in computed.items():
        label = spec.get("label", name) if isinstance(spec, dict) \
            else str(spec)
...
def load_system_prompt() -> str:
    """Return ``prompt.md`` with the canonical schema injected.
    ...
    if text.count(PLACEHOLDER) != 1:
        ...
        raise ValueError(f"{SYSTEM_PROMPT.name} must contain exactly one "
                         f"{PLACEHOLDER}, found {found}")
    schema = json.loads(SCHEMA_FILE.read_text(encoding="utf-8"))
    block = build_schema_block(schema)
    if not block.strip():
        raise ValueError("canonical-schema block rendered empty from "
                         f"{SCHEMA_FILE.name}")
    return text.replace(PLACEHOLDER, block)


def call_ollama(system: str, records: list[dict],
                url: str, model: str) -> dict:
    """POST the chat payload and return the server's JSON response.
    ...
    try:
        ...
        raise RuntimeError(f"Ollama HTTP {exc.code} from {url}: {detail}") \
            from exc
    ...
    try:
        ...
        snippet = body[:300]
        raise RuntimeError(f"Ollama at {url} returned non-JSON: "
                           f"{snippet!r}") from exc
...
def parse_response(body: dict) -> list[dict]:
    """Extract and parse the assistant's JSON array from a /api/chat reply.
    ...
    if isinstance(content, list):
        ...
        try:
            ...
            raise RuntimeError(
                f"model reply is not valid JSON: {content[:300]!r}") from exc
    if not isinstance(parsed, list):
        raise RuntimeError(f"model reply is a JSON {type(parsed).__name__}, "
                           f"expected an array: {content[:300]!r}")
    return parsed


def validate_decisions(records: list[dict],
                       decisions: list[dict]) -> dict[str, dict]:
    """Enforce the 4-field contract and return an id-keyed decision map.
    ...
    def bad(decision: dict, reason: str) -> None:
        rid = decision.get("id", "<missing id>") \
            if isinstance(decision, dict) else repr(decision)[:40]
    ...
    for position, decision in enumerate(decisions):
        ...
        if set(decision) != set(DECISION_KEYS):
            bad(decision, f"fields must be exactly {DECISION_KEYS}, "
        ...
        if target is None or transform is None:
            ...
            if not (isinstance(target, str) and target) \
                    or not (isinstance(transform, str) and transform):
                bad(decision,
                    "mapped lines need non-empty target and transform")
    ...
    if problems:
        raise RuntimeError("decision contract violations:\n  "
                           + "\n  ".join(problems))
    return by_id


def annotate(records: list[dict],
             by_id: dict[str, dict]) -> list[dict]:
    """Return input-ordered records extended with the decision fields.
...
def main() -> int:
    ...
    parser.add_argument("--out", default=None,
                        help="output path (default "
                             "data/reclassified/<cik>_<stmt>.json)")
    ...
    print(f"reclassified {stem}: {len(merged)} records "
